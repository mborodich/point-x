...
contract PointX {
	...
	function addAdmin(address acc) external {
		core.addAdmin(acc);
	}
	...
	function addUserAndUnlockTasks(address payable acc) external {
		increaseUserEthBalance(acc);
		core.addUserAndUnlockTasks(acc);
	}
	...
	// --------------------------- REWARD ---------------------------
	function addReward(string calldata caption, string calldata description, string calldata image, uint value, uint totalAmount) external {
		core.addReward(Core.NewReward(caption, description, image, value, totalAmount));
	}
	...
	// --------------------------- TASK ---------------------------
	function addTask(string calldata caption, string calldata description, string calldata image, uint8 itemType, bytes calldata data, uint value, uint totalAmount) external {
		core.addTask(Core.NewTask(caption, description, image, itemType, data, value, totalAmount));
	}
