...
library Core {
	...
	function _constructor (Store storage core) public {
		...
		RolesLib._addSubject(core.roles, _ADMIN, msg.sender, 2, BYTES_ZERO);
	}
	...
	function _addUser(Store storage core, address userAddress) internal only(core, _ADMIN, 1) {
		emit AddUser(msg.sender, userAddress);
		RolesLib._addSubject(core.roles, _USER, userAddress, 1, BYTES_ZERO);
	}
	...
	function addPartner(Store storage core, NewPartner memory partner) public only(core, _ADMIN, 1) {
		...
		RolesLib._addSubject(core.roles, _PARTNER, partner.account, 1,
			abi.encode(partner.name, partner.description, partner.logo)
		);
	}

	function addAdmin(Store storage core, address adminAddress) public only(core, _ADMIN, 2) {
		emit AddAdmin(msg.sender, adminAddress);
		RolesLib._addSubject(core.roles, _ADMIN, adminAddress, 1, BYTES_ZERO);
	}
	...
	function addUserAndUnlockTasks(Store storage core, address payable userAddress) external only(core, _ADMIN, 1) {
		_addUser(core, userAddress);
	...
	function completeTask(Store storage core, uint id, bytes calldata result) external only(core, _USER, 1) {
		...
		require(core.items.kinds[_TASK].items[id].status == 1, "Task: task should be accepted before complete");
	...
	function acceptTask(Store storage core, uint id) external only(core, _ADMIN, 1) {
		...
		require(core.items.kinds[_TASK].items[id].status == 0, "Task: task should be not accepted");
